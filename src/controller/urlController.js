    ...
    try {
        ...
        if (cachedData) {
            ...
            if (checkRes) {
                ...
                await redisService.SET_ASYNC(longUrl, JSON.stringify(url12), 'EX', 60 * 60 * 24)
                await redisService.SET_ASYNC(urlCode , url12.longUrl, 'EX', 60 * 60 * 24)
...
let getUrl = async function (req, res) {
    try {
        ...
        if (getRedisRes) {
            console.log("redis work...")
            return res.redirect(302, getRedisRes);
        }
        ...
        return res.redirect(302,urlCode.longUrl)
